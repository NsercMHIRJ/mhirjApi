...
def connect_database_for_scatter_plot_v22(from_dt, to_dt,aircraftno):
    sql = "SELECT * FROM Airline_MDC_Data WHERE aircraftno= "+str(aircraftno)+" AND DateAndTime BETWEEN '" + from_dt + "' AND '" + to_dt + "'" 
    column_names = ["Aircraft", "Tail", "Flight Leg No",
                    "ATA Main", "ATA Sub", "ATA", "ATA Description", "LRU",
                    "DateAndTime", "MDC Message", "Status", "Flight Phase", "Type",
                    "Intermittent", "Equation ID", "Source", "Diagnostic Data",
...
#scatter-------------------
@app.post("/api/scatter_chart_MDC_PM/{from_date}/{to_date}/{aircraftno}")
async def get_ScatterChart_MDC_PM_Data(from_date:str, to_date:str, aircraftno:int):
    MDCdataDF = connect_database_for_scatter_plot_v22(from_date, to_date, aircraftno)
...
async def generateReport(analysisType: str, occurences: int, legs: int, intermittent: int, consecutiveDays: int, ata: str, exclude_EqID:str, airline_operator: str, include_current_message: int, fromDate: str , toDate: str):
    ...
    if analysisType.lower() == "history":
        ...
        # return resObjJam.to_json(orient='records')
...
async def generateDeltaReport(analysisType: str, occurences: int, legs: int, intermittent: int, consecutiveDays: int,
        ...
        if (analysisType.lower() == "history"):
            listOfJams()
            curr_history = historyReport(occurences, legs, intermittent, consecutiveDays, ata, exclude_EqID, airline_operator, include_current_message, curr_fromDate, curr_toDate)
            prev_history = historyReport(occurences, legs, intermittent, consecutiveDays, ata, exclude_EqID, airline_operator, include_current_message, prev_fromDate, prev_toDate)
            ...
            for item in prev_history_nums.values:
                ...
                if i < prev_history.loc[True_list].values.size:
                    if item[0] in prev_history.loc[True_list].values:     
                        ...
                        'Consecutive FL': item[11], 'Intermittent': item[12], 'Date From': str(item[13]), 'Date To': str(item[14]), 'Reasons For Flag': item[15],
                        'Priority': item[16], 'MHIRJ Known Message': item[17], 'Mel or No-Dispatch': item[18], 'MHIRJ Input': item[19], 'MHIRJ Recommended Action': item[20], 
                        'MHIRJ Additional Comment': item[21], 'Jam': item[22], 'backgroundcolor': '#fde9d9'}
